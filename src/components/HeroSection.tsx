...
export default function HeroSection({ onSectionChange }: HeroSectionProps) {
  ...
  const [highlights, setHighlights] = useState([
    { game: 'LOL', badge: '🔴 LIVE', title: 'LPL 2026夏季赛', match: 'JDG vs BLG', viewers: '123万在线', accent: '#C89B3C' },
    { game: 'VALORANT', badge: '🔴 LIVE', title: 'VCT 2026', match: 'EDG vs ZETA', viewers: '85万在线', accent: '#FF4655' },
    { game: 'CS2', badge: '⏰ UPCOMING', title: 'IEM Cologne', match: 'NAVI vs VIT', viewers: '今晚 20:00', accent: '#FF6B35' },
  ]);
    ...
    const load = () => fetchLiveMatches().then(all => {
      ...
      const newHighlights = live.slice(0, 3).map(r => {
        ...
        return {
          ...
          viewers: r.viewers
            ? lang === 'zh'
              ? `${(r.viewers / 10000).toFixed(0)}万在线`
              : `${(r.viewers / 1000).toFixed(1)}K watching`
            : t('stream.live'),
      ...
      if (newHighlights.length < 3) {
        upcoming.slice(0, 3 - newHighlights.length).forEach(r => {
          ...
          newHighlights.push({
            ...
            title: r.tournament || r.game,
            match: `${r.teamA.shortName} vs ${r.teamB.shortName}`,
            viewers: r.startTime || t('stream.upcoming'),
            accent: game?.color || '#6366f1',
          });
        });
      }
      ...
      setStats([
        { label: t('hero.stats.live'), value: String(live.length), unit: t('hero.stats.matches') },
        { label: t('hero.stats.today'), value: String(all.length), unit: t('hero.stats.battles') },
        { label: t('hero.stats.online'), value: totalViewers >= 10000 ? String(Math.round(totalViewers / 10000)) : String(totalViewers || '0'), unit: totalViewers >= 10000 ? t('hero.stats.viewers') : '' },
        { label: t('hero.stats.covering'), value: String(TOURNAMENTS.length), unit: t('hero.stats.tournaments') },
      ]);
    }).catch(() => {});
    load();
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-32 w-full">
        <div className="max-w-3xl">
          ...
          <h1 className="text-5xl md:text-7xl font-black text-white mb-4 leading-tight" style={{ fontFamily: 'Rajdhani, Noto Sans SC, sans-serif' }}>
            {heroTitleParts[0] && (
              <span style={{ color: '#a5b4fc' }}>{heroTitleParts[0]}</span>
            )}
            {heroTitleParts[1] && (
              <>
                <br />
