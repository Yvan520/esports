...
export default function Navbar({ activeGame, onGameChange, activeSection, onSectionChange }: NavbarProps) {
  ...
  const sections = [
    ...
    { id: 'tournaments', label: t('nav.tournaments') },
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => onSectionChange('home')}>
            ...
            <div>
              ...
              <div className="text-gray-400 text-xs leading-none">{t('nav.subtitle')}</div>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <button
              onClick={toggleLang}
              className="flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold cursor-pointer transition-all hover:scale-105"
              style={{
            ...
            >
              <span style={{ color: lang === 'zh' ? '#a5b4fc' : '#6b7280', fontWeight: lang === 'zh' ? 800 : 400 }}>中</span>
              <span className="text-gray-600">|</span>
              <span style={{ color: lang === 'en' ? '#a5b4fc' : '#6b7280', fontWeight: lang === 'en' ? 800 : 400 }}>EN</span>
            </button>
      ...
      {menuOpen && (
        <div className="md:hidden border-t px-4 py-4 flex flex-col gap-2" style={{ background: 'rgba(8,8,20,0.98)', borderColor: 'rgba(99,102,241,0.2)' }}>
          ...
          <div className="flex items-center gap-2 mt-2">
            <button
              onClick={toggleLang}
              className="flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-bold cursor-pointer"
              style={{
                background: 'linear-gradient(135deg, rgba(99,102,241,0.25), rgba(139,92,246,0.25))',
                border: '1px solid rgba(99,102,241,0.5)',
                color: '#c4b5fd',
            ...
            >
              <span style={{ color: lang === 'zh' ? '#a5b4fc' : '#6b7280', fontWeight: lang === 'zh' ? 800 : 400 }}>中文</span>
              <span className="text-gray-600 mx-1">|</span>
              <span style={{ color: lang === 'en' ? '#a5b4fc' : '#6b7280', fontWeight: lang === 'en' ? 800 : 400 }}>EN</span>
            </button>
            <button className="flex-1 py-2 text-sm text-gray-300 rounded-lg cursor-pointer" style={{ background: 'rgba(255,255,255,0.06)' }}>{t('nav.login')}</button>
            <button className="flex-1 py-2 text-sm text-white rounded-lg font-medium cursor-pointer" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>{t('nav.register')}</button>
          </div>
        </div>
      )}
    </nav>
